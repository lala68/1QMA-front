      ...
      setTimeout(() => {
        this.route.queryParams.subscribe(async (params) => {
          ...
          if (this.generalService.userObj) {
            this.generalService.onFontSelect(
              this.generalService.userObj?.preferedFont
            );
          }
          ...
          if (status == 1) {
            if (user_id) {
              ...
              await Preferences.set({
                key: 'account',
                value: JSON.stringify({ _id: user_id }),
              });
            }
            await this.authService.isAuthenticated();
            await this.generalService.getUserData();
            this.authService.getUserDetails(this.generalService.userId).then(
              async (user) => {
                await Preferences.remove({ key: 'account' });
                await Preferences.set({
                  key: 'account',
            ...
            if (
              currentPath.includes('signup-refer-email') ||
            ...
            if (invitation_id) {
              if (!this.generalService.userId) {
                this.authService
                  .registerInvitationLink(invitation_id)
                  .then((data) => {
                    if (data.status == 1) {
              ...
              if (this.generalService.userId) {
                this.clientService.clientInit().then(async (data) => {
                  ...
                  this.generalService.hasCompletedSignup =
                    data.data.user.hasCompletedSignup;
                  this.generalService.emailVerified =
                  ...
                  if (
                    ...
                    this.authService
                      .getUserDetails(this.generalService.userId)
                      .then(
                        async (user) => {
                          await Preferences.remove({ key: 'account' });
                          await Preferences.set({
                          ...
                          if (user.data.inWaitList) {
                            ...
                            if (user.data.hasCompletedSignup) {
                              this.gameService.gameInit().then(
                                ...
                                (error) => {
                                  return this.processHTTPMsgService.handleError(
                                    error
                                  );
                                }
                              );
                    ...
                    //   this.generalService.clientInit = data.data;
                    //   this.generalService.userObj = (data.data.user);
                    await this.translateService.setDefaultLang(
                      this.generalService.userObj?.preferedLanguage?.code
                    );
                    document.documentElement.dir =
                      ...
                      'fa'
                        ? 'ltr'
                        : 'rtl';
                    this.generalService.direction =
                      document.documentElement.dir;
                    const bootstrapRTL = document.getElementById(
                      'bootstrapRTL'
                    ) as HTMLLinkElement;
                    bootstrapRTL.disabled =
                    ...
                    if (this.generalService.userObj.hasSeenIntros?.tutorial) {
                      this.router.navigate([
                        ...
                        this.router.url === '/wizard' ||
                        this.router.url === '/signup-social' ||
                        this.router.url === '/signup-refer-email' ||
                        this.router.url === '/social/callback'
                          ? '/dashboard'
                          : this.location.path(),
                      ]);
                      // await this.generalService.useGoogleTranslate();
